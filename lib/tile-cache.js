import mlasq from 'mlasq';

export { default as supported } from './is-supported.js';

const db = mlasq('tile-cache', ['tiles', 'fonts', 'data', 'images']);

const stores = {
  tile: db.store('tiles'),
  font: db.store('fonts'),
  json: db.store('data'),
  image: db.store('images')
};

export function put(name, key, data) {
  return stores[name].put(key, data);
}

export function get(name, key) {
  return stores[name].get(key);
}

export function remove(name, key) {
  return stores[name].remove(key);
}

export async function check(name, key) {
  const store = stores[name];
  const count = await store.count(key);
  return count > 0;
}

export function drop(name) {
  return stores[name].clear();
}
